...
> 🎬 **デモ動画は準備中です** — 録画完了後、動画プラットフォーム（ビリビリ / YouTube）へのリンクをここに掲載し、約 3 分のワークフロー紹介をご覧いただけます。
>
> 💡 待ちきれない方は、リポジトリをクローンして 5 分でローカル実行できます（アカウントや API キーは不要）—「ローカル開発」参照。
...
![コンテンツ生産パイプライン](./docs/workflow.svg)

要点:

- **単一 Worker・マルチテナント**:認証・リスク分類・生成・課金・監査などのビジネスロジックはすべてサーバー側で実行され、ブラウザは権威状態を保持しません;
- **D1 が正典**:15 テーブルを Drizzle ORM で管理し、マイグレーションは `drizzle/` に配置;
- **Cron 同期**:コネクタ同期は 30 分ごとに実行され、指数バックオフとレート制限による一時停止に対応;
- **暗号化された認証情報**:コネクタのキー/トークンは AES-256-GCM で暗号化し、キーは `wrangler secret` で注入;
- **コンプライアンスループ**:生成 → 承認 → 書き出しがすべて監査され、書き出しには出典リスト・事実リスト・AI 表示・バージョン履歴が含まれます。
...
http://localhost:3000（自分のマシン上のみで見られるローカルアドレス）を開きます。デフォルトはデモユーザー + デモエンジンで、**API キーやアカウントは不要です**:
